...
export default function CreateAssetPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setErrors([serverMsg || 'Something went wrong while connecting to LegacyOS. Please try again.']);
      window.scrollTo(0, 0);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-extrabold text-slate-950 tracking-tight">Add Digital Asset</h1>
        <p className="mt-1 text-slate-500 text-sm">Organize a new document, credential reference, message, or personal instruction.</p>
      </div>
      ...
      {errors.length > 0 && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-5 mb-8">
          <div className="flex gap-2">
            ...
            <h4 className="text-sm font-bold text-red-800">Please correct the following:</h4>
          </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white border border-slate-200 rounded-2xl p-6 md:p-8 shadow-sm space-y-6">
        ...
        <div>
          ...
          <input
            id="assetName"
            type="text"
            placeholder="e.g. Homeowners Insurance Policy, Google Password Reference"
            value={name}
